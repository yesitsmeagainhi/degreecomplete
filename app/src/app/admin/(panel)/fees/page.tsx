...
export default async function FeesPage({ searchParams: searchParamsPromise }: { searchParams: Promise<{ status?: string; university?: string; flagged?: string }> }) {
  const searchParams = await searchParamsPromise;
  await requireStaff("admin.fees");
  const [fees, universities] = await Promise.all([
    dbAdmin.feeRecord.findMany({
      ...
      orderBy: [{ program: { university: { name: "asc" } } }, { program: { course: "asc" } }, { specialization: "asc" }], take: 400,
  ...
  return (
    <div>
      ...
      <p className="muted mt-3">{fees.length} records</p>
