...
export function CompareButton({ id, label }: { id: string; label: string }) {
  ...
  return (
    <button type="button" onClick={() => toggle(id, label)} aria-pressed={on} className={`inline-flex items-center rounded-md px-2.5 py-1 text-xs font-medium transition-colors ${on ? "bg-navy text-white hover:bg-navy-700" : "text-gray-500 hover:text-navy hover:bg-gray-100"}`}>
      {on ? "✓ Added" : "+ Compare"}
    </button>
  );
}
